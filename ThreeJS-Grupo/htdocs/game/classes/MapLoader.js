			...
			{   
				...
				{	
					object.traverse( function ( o ) {
						...
						//console.log(loaderMesh); //the 3 meshes are fine here
		...
		switch(mapID)
		{
			case 1:
			{
				...
				for(var i = 0; i < 64; i++){
					...
					//TO DO: we must grab the same geometries and merge'em	
						if(FirstCon.includes(FirstCol[count]))//add the preloaded objects to group
						{
							...
							//this: var geometryB = new THREE.Geometry().fromBufferGeometry( bufferedMesh.geometry );

							//bufferedMesh.rotation.y *= Math.random();
							...
							{  
								...
								//get positions
								var arrayOfPositions = Array.prototype.slice.call(objMerge[indexNow].attributes.position.array);	
								var offset = arrayOfPositions.length/3;
								var arrayOfPositions2 = Array.prototype.slice.call(bufferedMesh.geometry.attributes.position.array);
								...
								//concat positions
								arrayOfPositions = arrayOfPositions.concat( arrayOfPositions2 );
								
								//to float32
								var attributePositions = new Float32Array( arrayOfPositions );
								
								var f32Positions = new Float32Array( attributePositions );
								var PositionsBuffer = new THREE.BufferAttribute(f32Positions, 3);
								
								geo.addAttribute("position", PositionsBuffer);
								
								//get normals
								var arrayOfNormals = Array.prototype.slice.call(objMerge[indexNow].attributes.normal.array);	
								var arrayOfNormals2 = Array.prototype.slice.call(bufferedMesh.geometry.attributes.normal.array);
								//concat normals
								arrayOfNormals = arrayOfNormals.concat(arrayOfNormals2);
								//to float32
								var attributeNormal = new Float32Array( arrayOfNormals );
								
								var f32norm = new Float32Array( attributeNormal );
								var normBuffer = new THREE.BufferAttribute(f32norm, 3);
								
								geo.addAttribute("normal", normBuffer);
								
								//get uvs
								var arrayOfUvs = Array.prototype.slice.call(objMerge[indexNow].attributes.uv.array);	
								var arrayOfUvs2 = Array.prototype.slice.call(bufferedMesh.geometry.attributes.uv.array);
								//concat uvs
								arrayOfUvs = arrayOfUvs.concat(arrayOfUvs2);
								//to float32
								var attributeUvs = new Float32Array( arrayOfUvs );
								
								var f32Uvs = new Float32Array( attributeUvs );
								var UvsBuffer = new THREE.BufferAttribute(f32Uvs, 2);
								
								geo.addAttribute("uv", UvsBuffer);
								
								
								//get indexes
								var arrayOfIndex = Array.prototype.slice.call(objMerge[indexNow].getIndex().array);	
								
								var arrayOfIndex2 = Array.prototype.slice.call(bufferedMesh.geometry.getIndex().array);
								...
								//concat indexes
								arrayOfIndex = arrayOfIndex.concat( arrayOfIndex2 );
								
								//to uint32
								var u32Index = new Uint32Array( arrayOfIndex );
								//console.log(u32Index);
								var IndexBuffer = new THREE.BufferAttribute(u32Index, 1);
                ...
                mapMesh.material.displacementBias = - 0.428408;			
				//end tile system 3/////////////////////////
